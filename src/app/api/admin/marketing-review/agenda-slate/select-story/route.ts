...
/**
 * Persist human Story selection and requeue production (QUEUED).
...
export async function POST(request: Request) {
  ...
  const parsed = agendaSlateSelectStorySchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    ...
    const result = await service.selectStoryAndResumeProduction({
