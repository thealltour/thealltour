/**
 * 발췌용: src/components/products/ProductCard.tsx 전체 원문
 * PR 검토용 문서(docs/product-list-card-excerpt-for-pr.md) 5번에서 참조.
 */
// @see docs/product-list-card-excerpt-for-pr.md 섹션 5
...
function badgeVariantToChipStyle(variant: "accent" | "muted" | "gold") {
  if (variant === "accent") return "border-blue-200 bg-blue-600/95 text-white";
  if (variant === "gold") return "border-amber-200 bg-amber-500/95 text-white";
...
export default function ProductCard({
  ...
  const topLeftChipsRaw = [statusChip, categoryChip ?? themeChip, ...badgeChips]
    .filter((x): x is { label: string; variant: "accent" | "muted" | "gold" } => Boolean(x))
  ...
  const cardContent = isRelatedLayout ? (
    relatedCardContent
  ) : (
    <div className="flex min-h-[140px] w-full items-stretch">
      <div
        className={cn(
          ...
          isListLayout ? "w-[38%] min-w-[180px] max-w-[280px]" : "w-[42%] min-w-[140px] max-w-[220px]",
        )}
      >
        <div className="absolute left-2 top-2 z-10 flex flex-wrap gap-1">
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden p-4">
        <div className="min-w-0">
          <div className={cn("relative overflow-hidden", "min-h-[1.5rem]")}>
            <h2
              className={cn(
        ...
        <div className="mt-auto flex flex-col gap-2 pt-3">
          ...
          {hrefDetail ? (
            <div className="flex justify-end">
              <span
                className="inline-flex items-center gap-0.5 text-[10px] font-medium text-[var(--primary)] opacity-90 group-hover:opacity-100"
                aria-hidden
              >
                자세히 보기
                <svg
