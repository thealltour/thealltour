...
export type RelatedThemeSectionProps = {
  items: ProductTaxonomy[];
  subtitle?: string;
  query?: string | null;
};

const FALLBACK_IMAGE = "https://picsum.photos/seed/thealltour-theme/800/500";

export default function RelatedThemeSection({
  items,
  subtitle,
  query,
}: RelatedThemeSectionProps) {
  if (items.length === 0) return null;

  return (
    <section aria-labelledby="related-themes-heading" className="space-y-4">
      <div>
        <h2
      ...
      >
        {items.map((item, index) => {
          const href = getThemeLandingHref(item);
