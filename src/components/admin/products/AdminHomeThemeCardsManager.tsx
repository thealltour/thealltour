...
const DEFAULT_THEME_EYEBROW = "TRAVEL THEMES";
const DEFAULT_THEME_TITLE = "이런 여행은 어떠세요?";
const DEFAULT_THEME_DESCRIPTION = "테마별로 여행 상품을 둘러보세요.";
...
export default function AdminHomeThemeCardsManager() {
  ...
  const [sectionEyebrow, setSectionEyebrow] = useState(DEFAULT_THEME_EYEBROW);
  const [sectionTitle, setSectionTitle] = useState(DEFAULT_THEME_TITLE);
  const [sectionDescription, setSectionDescription] = useState(DEFAULT_THEME_DESCRIPTION);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (settingsRes.ok && settingsData && !("message" in settingsData)) {
        ...
        const settings = {
          home_theme_card_ids: raw.home_theme_card_ids ?? "[]",
          home_theme_section_eyebrow: raw.home_theme_section_eyebrow ?? "",
          home_theme_section_title: raw.home_theme_section_title ?? "",
          home_theme_section_description: raw.home_theme_section_description ?? "",
        } as Pick<
          SiteSettings,
          "home_theme_card_ids" | "home_theme_section_eyebrow" | "home_theme_section_title" | "home_theme_section_description"
        >;
        const ids = parseHomeThemeCardIds(settings);
        setOrderedIds(ids.slice(0, MAX_HOME_CARDS));
        setSectionEyebrow(settings.home_theme_section_eyebrow ?? "");
        setSectionTitle(settings.home_theme_section_title ?? "");
        setSectionDescription(settings.home_theme_section_description ?? "");
      } else {
        setOrderedIds([]);
      }
    } catch (err) {
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch("/api/admin/site-settings", {
        ...
        body: JSON.stringify({
          home_theme_card_ids: JSON.stringify(orderedIds),
          home_theme_section_eyebrow: sectionEyebrow.trim(),
          home_theme_section_title: sectionTitle.trim(),
          home_theme_section_description: sectionDescription.trim(),
        }),
      });
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 메인 홈 테마 섹션 문구 (상단 라벨·제목·부제목) */}
      <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4 space-y-4">
        <h3 className="text-sm font-semibold text-[var(--text-primary)]">메인 홈 테마 섹션 문구</h3>
        <p className="text-xs text-[var(--text-muted)]">
          메인 페이지 테마 카드 섹션 상단에 보이는 텍스트입니다. 비우면 해당 항목은 메인에서 표시되지 않습니다.
        </p>
        <div className="grid gap-3 sm:grid-cols-1">
          <label className="block">
            <span className="mb-1 block text-xs font-medium text-[var(--text-muted)]">상단 라벨 (예: TRAVEL THEMES)</span>
            <input
              type="text"
              value={sectionEyebrow}
              onChange={(e) => setSectionEyebrow(e.target.value)}
              placeholder={DEFAULT_THEME_EYEBROW}
          ...
          <label className="block">
            ...
            <input
              ...
              placeholder={DEFAULT_THEME_TITLE}
              className="w-full rounded-lg border border-[var(--border)] bg-[var(--surface-muted)] px-3 py-2 text-sm text-[var(--text-primary)] outline-none focus:border-[var(--primary)]"
              aria-label="테마 섹션 제목"
            />
          </label>
