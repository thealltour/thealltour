...
import { createContentMemorySource } from "@/lib/marketing/memory/sources/contentMemorySource";
import type { MemoryDocument, MemoryIngestionResult, MemoryStore, MemoryWriteResult } from "@/lib/marketing/memory/types";

const PREVIEW_CHARS = 500;

export type RunContentMemoryIngestionInput = {
  contentId?: string;
  productId?: string;
  apply: boolean;
  preview?: boolean;
...
function resolveDryRun(input: RunContentMemoryIngestionInput): boolean {
...
export async function runContentMemoryIngestion(input: RunContentMemoryIngestionInput): Promise<{
  dryRun: boolean;
  ingested: boolean;
  found: boolean;
  ...
  const source = createContentMemorySource();
  const documents = input.loadDocuments
    ...
    : await source.load({
        ...
        limit: input.contentId ? 1 : 20,
      });

  if (documents.length === 0) {
    log("content memory not found / insufficient text");
    if (input.contentId) log(`content id: ${input.contentId}`);
    if (input.productId) log(`product id: ${input.productId}`);
  ...
  log(`source count: ${documents.length}`);
  for (const document of documents) {
    ...
    log(`source: ${typeof meta.source === "string" ? meta.source : document.sourceType}`);
    log(`content id: ${typeof meta.contentId === "string" ? meta.contentId : document.sourceId}`);
    log(`memory type: ${document.memoryType}`);
    log(`source type: ${document.sourceType}`);
    log(`source id: ${document.sourceId}`);
    if (Array.isArray(meta.channels)) log(`channels: ${meta.channels.join(", ")}`);
