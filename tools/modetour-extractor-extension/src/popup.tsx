...
import { buildModetourImportV1 } from "~lib/buildImport";

/** 주소창에 https://www. 가 생략되어 보여도 tab.url은 전체 URL입니다. http·www 없는 경우도 허용 */
function isModetourPackageUrl(url: string): boolean {
  try {
    const u = new URL(url);
    const host = u.hostname.toLowerCase();
    const path = u.pathname.toLowerCase();
    if (!host.includes("modetour.com")) return false;
...
function useExtract() {
  const [data, setData] = useState<ModetourImportV1 | null>(null);
  ...
  const extract = useCallback(async () => {
    ...
    try {
      ...
      const url = tab.url ?? "";
      if (!isModetourPackageUrl(url)) {
        setError("이 페이지는 모두투어 상품 페이지가 아닙니다. 주소에 modetour.com/package/... 가 포함되는지 확인하세요.");
        return;
      }
      ...
      setError(
        "이 페이지는 모두투어 상품 페이지가 아니거나 DOM 추출에 실패했습니다.",
      );
...
function Popup() {
  ...
  const toCopyData = (): ModetourImportV1 | null => {
    if (!data) return null;
  ...
  return (
    <div style={{ width: 360, minHeight: 320, padding: 16, fontFamily: "sans-serif" }}>
      <h2 style={{ margin: "0 0 12px", fontSize: 16 }}>모두투어 상품 추출</h2>
      ...
      {data && (
        <>
          ...
          >
            ...
            {meta?.uiPrep && (
              <div style={{ fontSize: 11, color: "#64748b" }}>
                UI 준비: 탭 클릭 {meta.uiPrep.didClickTab ? "O" : "X"} / 아코디언 {meta.uiPrep.expandedCount}개 펼침
              </div>
            )}
          ...
          {data.warnings && data.warnings.length > 0 && (
            <div style={{ marginBottom: 12 }}>
              ...
              {(() => {
                const required: ModetourImportWarning[] = [];
                const caution: ModetourImportWarning[] = [];
                const other: ModetourImportWarning[] = [];
      ...
      <p style={{ margin: "12px 0 0", fontSize: 11, color: "#64748b" }}>
        추출 후 클립보드 복사 → 어드민 /admin/products/new-modetour 에 붙여넣기
      </p>
    </div>
  );
}
