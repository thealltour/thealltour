...
import { PERFORMANCE_MEMORY_DEFAULT_LOOKBACK_DAYS } from "@/lib/marketing/memory/constants";
import { MemoryValidationError } from "@/lib/marketing/memory/errors";

export type PerformanceMemoryCliArgs = {
  productId: string;
  apply: boolean;
  preview: boolean;
  dryRun: boolean;
...
export function parsePerformanceMemoryCliArgs(argv: string[]): PerformanceMemoryCliArgs {
  let productId: string | undefined;
  let apply = false;
  let dryRun = false;
  let preview = false;
  let lookbackDays = PERFORMANCE_MEMORY_DEFAULT_LOOKBACK_DAYS;
  let channel: string | undefined;
  let minEventCount: number | undefined;
  ...
  for (let index = 0; index < argv.length; ) {
    ...
    if (token === "--min-event-count" || token.startsWith("--min-event-count=")) {
      const read = readFlagValue(argv, index, "--min-event-count");
      minEventCount = parsePositiveInt(read.value, "--min-event-count");
      index += read.consumed;
      continue;
    }
