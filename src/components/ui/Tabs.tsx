...
export function Tabs({ value, onChange, className, children }: TabsProps) {
  return (
    <div
      className={cn(
        "flex w-full max-w-full flex-wrap items-center gap-1 rounded-[var(--radius-pill)] bg-[var(--surface-muted)] p-1",
        className,
      )}
      data-value={value}
    >
...
export function TabsTrigger({
  ...
  return (
    <button
      type="button"
      className={cn(
        "min-h-[44px] shrink-0 rounded-[var(--radius-pill)] px-3 py-2 text-xs font-semibold leading-snug transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--surface-muted)] sm:px-4 sm:text-sm",
        selected
