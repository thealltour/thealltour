...
type UseInquiryMessageSendOptions = {
  inquiryId: string;
  onSent?: () => void;
  onThreadRefetch?: () => Promise<void>;
};

export function useInquiryMessageSend({
  inquiryId,
  onSent,
  onThreadRefetch,
}: UseInquiryMessageSendOptions) {
  ...
  const sendMessage = useCallback(
    async (input: { receiver: string; message: string; onMessageClear?: () => void }) => {
      if (!inquiryId || inFlightRef.current || sending) return false;
      ...
      const fp = createMessageFingerprint({ inquiryId, phone: to, text: body });
      if (
        isDuplicateSendCandidate({
          nextFingerprint: fp,
          lastSentFingerprint,
      ...
      try {
        const res = await fetch(`/api/admin/inquiries/${encodeURIComponent(inquiryId)}/send-message`, {
          method: "POST",
        ...
        console.debug("[useInquiryMessageSend] send failed", e);
