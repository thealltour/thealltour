...
function splitByDayBlocks(itineraryText: string): { num: number; start: number; end: number }[] {
  ...
  for (let i = 0; i < bestBlocks.length; i++) {
    const end = i + 1 < bestBlocks.length ? bestBlocks[i + 1].start : text.length;
    result.push({
...
function checkDaySequence(days: DayBlock[], warnings: HanatourImportWarning[]): void {
...
export function parseItineraryText(itineraryText: string): {
  itinerary: HanatourImportV1["itinerary"];
  ...
  const itinerary: HanatourImportV1["itinerary"] = {
    days: days.map((d) => ({
