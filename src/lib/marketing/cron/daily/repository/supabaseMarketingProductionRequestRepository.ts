...
import { buildRequeuedFailedRequest, buildRequeuedAwaitingStorySelectionRequest } from "@/lib/marketing/cron/daily/repository/createMarketingProductionRequestRepository";
import type {
  ...
  implements MarketingProductionRequestRepository
{
  ...
  async requeueAwaitingStorySelection(input: {
    ...
    const existing = await this.findByLogicalKey(input.logicalRunKey);
    if (!existing) throw new Error(`production request not found: ${input.logicalRunKey}`);
    const requeued = buildRequeuedAwaitingStorySelectionRequest(
      existing,
      input.now ?? new Date(),
    );
    return this.update(requeued);
