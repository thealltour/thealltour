 ...
 *
 * 실행:
 *   npx tsx scripts/ingest-content-memory.ts --content-id 실제-uuid
 *   npx tsx scripts/ingest-content-memory.ts --product-id 실제-uuid --channel threads --lookback-days 30
 *   npx tsx scripts/ingest-content-memory.ts --content-id 실제-uuid --apply
 *
...
async function main() {
  ...
  const outcome = await runContentMemoryIngestion(args);
  if (outcome.result.failed > 0) process.exit(1);
}

main().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : String(error);
  console.error(`content memory ingestion failed: ${message}`);
  process.exit(1);
});
