...
import type {
  ThreadsGenerateApiResponse,
  ThreadsGenerateModalProps,
  ThreadsPublishApiResponse,
} from "./threadsModal.types";
import {
...
export default function ThreadsGenerateModal({
  open,
  productId,
  productTitle,
  onClose,
  ...
  const [mode, setMode] = useState<ThreadsMarketingMode>("TIMEDEAL");
  ...
  const generate = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/admin/threads/generate", {
        ...
        body: JSON.stringify({ productId: productId.trim(), marketingMode: mode }),
      });
      const data = (await res.json()) as ThreadsGenerateApiResponse;
      if (!res.ok || !data.ok) {
  ...
  const publish = useCallback(async () => {
    if (!productId?.trim() || !draftContent.trim() || !targetKeyword.trim()) return;
    setPublishing(true);
    setError(null);
    try {
      ...
      const data = (await res.json()) as ThreadsPublishApiResponse;
      if (!res.ok || !data.ok) {
  ...
  }, [draftContent, imageUrl, onPublished, productId, targetKeyword]);

  if (!open) return null;

  const canPublish = Boolean(draftContent.trim() && targetKeyword.trim() && !loading && !publishing);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="threads-modal-title"
      onClick={onClose}
    >
      <div
        className="flex max-h-[min(92vh,800px)] w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface)] shadow-xl"
        onClick={(e) => e.stopPropagation()}
      >
        <header className="flex shrink-0 items-start justify-between gap-3 border-b border-[var(--border)] px-4 py-3">
          <div className="min-w-0">
            <h2
              id="threads-modal-title"
            ...
            <p className="mt-1 text-xs text-[var(--text-muted)]">
              Gemini로 초안을 만든 뒤 검수하고, 확인을 눌러야 Threads에 올라갑니다.
            </p>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3">
          ...
          <div className="mb-2 flex items-center justify-between gap-2">
            <label className="text-xs font-semibold text-[var(--text-primary)]" htmlFor="threads-draft">
              게시 원고
            </label>
          ...
          <textarea
            ...
            className="h-[220px] w-full resize-y rounded-lg border border-[var(--border)] bg-[var(--surface-muted)] p-3 text-sm leading-relaxed text-[var(--text-primary)] whitespace-pre-wrap"
            spellCheck={false}
