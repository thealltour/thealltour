...
import { setChannelReviewStatusSchema } from "@/lib/marketing/review/validation";

export const dynamic = "force-dynamic";

type RouteContext = { params: Promise<{ candidateId: string }> };

/** CG-4C — approve / skip / reset one channel. */
export async function POST(request: Request, context: RouteContext) {
  ...
  const parsed = setChannelReviewStatusSchema.safeParse(body);
  if (!parsed.success) {
