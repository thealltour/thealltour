/**
 * 이미지 URL 대량 추출 북마클릿 (크롬 등)
 *
 * 설치 방법 (3줄):
 * 1. 아래 "북마클릿 한 줄" 전체를 복사합니다.
 * 2. 브라우저에서 새 북마크를 추가하고, URL 입력란에 붙여넣습니다.
 * 3. 이름은 예: "이미지 URL 추출" 로 저장한 뒤, 이미지가 많은 페이지(예: 모두투어 상품 페이지)에서 북마크를 클릭해 실행합니다.
 *
 * 수집 로직:
 * - img[src], img[data-src], img[data-original], img[data-lazy], img[data-srcset](srcset은 가장 큰 해상도 1개만)
 * - 인라인 style 또는 computed style의 background-image: url(...)
 * - a[href] 중 확장자가 .jpg/.jpeg/.png/.webp/.gif 로 끝나는 링크
 * 수집 후 URL 정규화(앞뒤 따옴표·괄호·공백 제거), http/https만 유지, 중복 제거하여 줄바꿈(\n)으로 이어 붙인 문자열을 클립보드에 복사합니다.
 *
 * 클립보드 실패 시:
 * - navigator.clipboard.writeText 가 실패하면(권한/비 HTTPS 등) prompt 창에 결과 텍스트를 띄워, 사용자가 Ctrl+A 후 복사할 수 있게 합니다.
 * - 그 다음 alert("총 N개 복사 완료 (창에서 복사해 주세요)") 로 안내합니다.
 */

// prettier-ignore
const BOOKMARKLET_ONE_LINE =
