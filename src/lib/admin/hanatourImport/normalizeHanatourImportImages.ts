/**
 * PR-IMAGE-3: HanatourImportV1 내 http(s) 이미지 URL을 Supabase product-images로 재호스팅.
 * 클라이언트 번들에 포함되지 않도록 server-only.
 */
import "server-only";

import type { HanatourImportV1, HanatourImageHeuristicHints } from "@/types/hanatourImport";
import { uploadImageFromUrl } from "@/lib/images/uploadImageFromUrl";
import { isHanatourUiStockImageUrl, upgradeHanatourImageUrl } from "@/lib/images/upgradeHanatourImageUrl";
...
function collectUniqueExternalImageUrls(input: HanatourImportV1): string[] {
  const raw: string[] = [];
...
function upgradeUrlField(u: string | undefined): string | undefined {
  if (!u?.trim()) return u;
  if (isHanatourUiStockImageUrl(u)) return undefined;
  return upgradeHanatourImageUrl(u);
}

function upgradeHanatourImportImageUrls(input: HanatourImportV1): HanatourImportV1 {
  const out = structuredClone(input) as HanatourImportV1;
  if (out.media) {
    out.media.heroImageUrl = upgradeUrlField(out.media.heroImageUrl);
    if (out.media.galleryImageUrls?.length) {
      out.media.galleryImageUrls = out.media.galleryImageUrls
        .map((x) => upgradeUrlField(x))
        .filter((x): x is string => !!x);
    }
    if (out.media.unassignedImageUrls?.length) {
      out.media.unassignedImageUrls = out.media.unassignedImageUrls
        .map((x) => upgradeUrlField(x))
        .filter((x): x is string => !!x);
    }
  }
  if (out.itinerary?.days?.length) {
    for (const d of out.itinerary.days) {
      if (d.imageUrls?.length) {
        d.imageUrls = d.imageUrls.map((x) => upgradeUrlField(x)).filter((x): x is string => !!x);
      }
      for (const ev of d.events ?? []) {
        if (ev.imageUrls?.length) {
          ev.imageUrls = ev.imageUrls.map((x) => upgradeUrlField(x)).filter((x): x is string => !!x);
        }
      }
    }
  }
...
function applyReplacements(input: HanatourImportV1, urlMap: Map<string, string>): HanatourImportV1 {
  const out = structuredClone(input) as HanatourImportV1;
  if (out.media) {
    ...
    if (hints && Object.keys(hints).length > 0) {
      const next: Record<string, HanatourImageHeuristicHints> = {};
 ...
 */
export async function normalizeHanatourImportImages(
  input: HanatourImportV1,
): Promise<{ payload: HanatourImportV1; stats: NormalizeImportImageStats }> {
  const upgradedInput = upgradeHanatourImportImageUrls(input);

  if (!process.env.SUPABASE_SERVICE_ROLE_KEY || !process.env.NEXT_PUBLIC_SUPABASE_URL) {
    return {
      payload: upgradedInput,
      stats: {
        uniqueUrls: 0,
        attempted: 0,
        uploaded: 0,
  ...
  const unique = collectUniqueExternalImageUrls(upgradedInput);
  const skippedInternal =
    (() => {
      ...
      visit(upgradedInput.media?.heroImageUrl);
      for (const u of upgradedInput.media?.galleryImageUrls ?? []) visit(u);
      for (const u of upgradedInput.media?.unassignedImageUrls ?? []) visit(u);
      for (const d of upgradedInput.itinerary?.days ?? []) {
  ...
  const payload = applyReplacements(upgradedInput, urlMap);
