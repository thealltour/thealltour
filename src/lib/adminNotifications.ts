...
import { dispatchAdminWebPush } from "@/lib/adminWebPush";
...
type AdminNotificationInsertRow = {
  type: string;
  title: string;
  message: string;
  target_url: string | null;
...
async function insertAdminNotificationIfNew(row: AdminNotificationInsertRow): Promise<boolean> {
  const { error } = await supabaseAdmin.from("admin_notifications").insert(row);
  if (error?.code === "23505") return false;
  if (error) {
    console.error("[adminNotifications] insert failed:", error.message);
    return false;
  }
  return true;
}

async function insertAndPushAdminNotification(row: AdminNotificationInsertRow): Promise<void> {
  const inserted = await insertAdminNotificationIfNew(row);
  if (!inserted) return;

  await dispatchAdminWebPush({
    title: row.title,
    body: row.message,
    targetUrl: row.target_url,
    type: row.type,
  }).catch((error) => {
    console.error("[adminWebPush] dispatch failed:", error);
  });
}
...
export async function createNewMemberNotification(input: NewMemberNotificationInput) {
  await insertAndPushAdminNotification({
    type: "new_member",
    title: "신규 회원 가입",
...
export async function createNewInquiryNotification(input: NewInquiryNotificationInput) {
  ...
  await insertAndPushAdminNotification({
    ...
    target_url: `/theall_manager_only/inquiries?id=${input.inquiryId}`,
...
export { getUnreadNotificationCount } from "@/lib/adminNotificationCounts";
