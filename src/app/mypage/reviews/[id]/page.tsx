      ...
      <div className="space-y-6">
        ...
        <MyPageCard>
          <header className="mb-4 border-b border-[var(--border)] pb-4">
            <h1 className="type-h3 text-[var(--text-primary)]">{review.title}</h1>
            {review.summary ? <p className="mt-1 text-sm text-[var(--text-secondary)]">{review.summary}</p> : null}
          ...
          {hasDetailRatings ? (
            <SectionBlock surface="muted" padding="sm" className="mb-4">
              <h3 className="mb-2 type-caption font-medium text-[var(--text-muted)]">세부 평점</h3>
          ...
          {images.length > 0 ? (
            <div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
              {images.map((url, index) => (
                <div
                  key={`${url}-${index}`}
                  className="relative aspect-square overflow-hidden rounded-xl ring-1 ring-[var(--border)]"
                >
                  <Image
                    src={url}
                    alt={`후기 이미지 ${index + 1}`}
          ...
          {hasStructuredContent ? (
            ...
            <p className="whitespace-pre-wrap text-sm text-[var(--text-primary)]">{review.content}</p>
          )}
        </MyPageCard>
      </div>
    </MyPageLayout>
