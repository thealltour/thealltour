...
/**
 * Download only `cardnews/**` PNGs (and any other cardnews artifacts) as ZIP.
 */
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    const zip = await buildMarketingAssetPackageZip({
      ...
      relativePathPrefix: "cardnews/",
      zipFileNameSuffix: "cardnews",
    });
    ...
    if (error instanceof MarketingAssetPathError && /not found|no readable/i.test(error.message)) {
