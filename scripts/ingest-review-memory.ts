/**
 * Review memory ingestion.
 *
 * Default is dry-run (no DB write, no embedding).
 *
 * 실행:
 *   npx tsx scripts/ingest-review-memory.ts --product-id 실제-uuid
 *   npx tsx scripts/ingest-review-memory.ts --product-id 실제-uuid --preview
 *   npx tsx scripts/ingest-review-memory.ts --product-id 실제-uuid --apply
 *
...
async function main() {
  ...
  const { runReviewMemoryIngestion } = await import("../src/lib/marketing/memory/reviewMemoryIngestionRun");
  const args = parseProductMemoryCliArgs(process.argv.slice(2));
  const outcome = await runReviewMemoryIngestion(args);
  if (outcome.result.failed > 0) process.exit(1);
}

main().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : String(error);
  console.error(`review memory ingestion failed: ${message}`);
  process.exit(1);
});
