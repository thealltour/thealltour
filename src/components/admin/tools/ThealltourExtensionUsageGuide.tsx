...
const USAGE_STEPS = [
  ...
  },
  {
    ...
    body: [
      ...
      "진행이 특정 %에서 멈춘 것처럼 보이면: 우측 하단 진행판 하단의 로그 영역(모노스페이스 텍스트)을 확인하세요. 어떤 단계(부모 탭 순회/배경 API/콘텐츠 폴백)에서 시간이 걸리고 있는지 타임스탬프로 표시됩니다. 근접-매일 출발·다개월 상품은 정상적으로도 다소 시간이 걸릴 수 있습니다(완전성 우선 설계).",
      "0.2.33: 부모 탭 달력에서 월 이동(>) 버튼이 날짜 스트립의 다음 날짜(>) 버튼과 혼동되어 월이 넘어가지 않던 문제를 수정했습니다. 클릭 후에도 월이 바뀌지 않으면 즉시 멈추고 원인을 기록하도록 검증 로직도 추가했습니다.",
    ].join("\n"),
  },
] as const;
