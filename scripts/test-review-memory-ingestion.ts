/**
 * Review memory dry-run smoke test.
 *
 * Always dry-run. DB INSERT/UPDATE 없음. embedding 기본 호출 없음.
 *
 * 실행:
 *   npx tsx scripts/test-review-memory-ingestion.ts --product-id 실제-uuid
 *   npx tsx scripts/test-review-memory-ingestion.ts --product-id 실제-uuid --preview
 *
...
async function main() {
  ...
  const outcome = await runReviewMemoryIngestion({ ...args, apply: false, preview: true });
  if (outcome.result.failed > 0) process.exit(1);
}

main().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : String(error);
  console.error(`review memory dry-run failed: ${message}`);
  process.exit(1);
});
