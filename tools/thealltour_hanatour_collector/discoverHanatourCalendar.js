...
(function (global) {
  ...
  if (typeof chrome !== "undefined" && chrome.runtime?.onMessage) {
    chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
      if (message?.type !== "GET_DISCOVERED_CALENDAR") return false;
      const meta = message.meta ?? {};
      discoverHanatourCalendar(document, meta)
        .then((result) => sendResponse({ ok: true, result, captureCount: capturedPayloads.length }))
